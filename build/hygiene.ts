...
function createGitIndexVinyls(paths: string[]): Promise<VinylFile[]> {
	...
	const fns = paths.map((relativePath) => () =>
		new Promise<VinylFile | null>((c, e) => {
			...
			cp.exec(
				`git diff --cached --diff-filter=D --name-only -- ${relativePath}`,
				{ maxBuffer: 1024 },
				(diffErr, diffOut) => {
					if (diffErr) {
						return e(diffErr);
					...
					// If file is staged for deletion, skip it
					if (diffOut.trim() === relativePath) {
						return c(null);
					}
					...
					fs.stat(fullPath, (err, stat) => {
						...
						cp.exec(
							...
							{ maxBuffer: Math.max(stat.size * 2, 10 * 1024 * 1024), encoding: 'buffer' },
							(err, out) => {
								if (err) {
									// If git show fails, the file might be deleted or new - try reading from filesystem
									if (err.code === 128 || err.message?.includes('unknown revision') || err.message?.includes('maxBuffer')) {
										// For new files or files not in index, read from filesystem
										fs.readFile(fullPath, (readErr, contents) => {
											if (readErr) {
